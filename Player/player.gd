...
signal player_died#向game管理器发送玩家死亡信号，再通过game管理器向玩家摄像机传递死亡信号
...
const KNOCKBACK_AMOUNT:=512.0#击退效果

@export var can_combo:=false#连击
...
var flash_tween: Tween#用于控制闪烁效果的Tween实例
...
func transition_state(from:State, to:State)->void:
	...
	match to:
		...
		State.HURT:
			...
			flash_tween.tween_property(graphics, "modulate", Color(1, 1, 1, 0.2), 0.1)
			flash_tween.tween_property(graphics, "modulate", Color.WHITE, 0.1)

		State.DYING:
			stop_flashing()  # 调用停止闪烁的函数, 停止闪烁并恢复颜色
...
func stop_flashing() -> void:
	if flash_tween:
		flash_tween.kill()  # 停止闪烁动画
		flash_tween = null  # 清空Tween实例
	graphics.modulate = Color.WHITE  # 恢复为正常颜色
